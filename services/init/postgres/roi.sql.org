...
*** Projections
    ...
    #+begin_src sql
      ...
      LANGUAGE PLPGSQL AS $$
        ...
        res float;
        begin
          select res.size into strict sz
        select
          (st_xmin(i.angles)+x*sz*angle_inc)*sat_height as west,
          (st_ymax(i.angles)-(y+st_metadata(rast).height)*sz*angle_inc)*sat_height as south,
          (st_xmin(i.angles)+(x+st_metadata(rast).width)*sz*angle_inc)*sat_height as east,
          (st_ymax(i.angles)-y*res*angle_inc)*sz*sat_height as north,
          angle_inc*sat_height as resolution
            from abi join abibands b using (resolution_id)
            join abi_fixed_image i using (goes_id)
           where block.satellite=abi.goes_id and block.band=b.band and i.image_id=block.product
    ...
    #+begin_src sql :tangle no
      create or replace function projected_rast (
      in b goes.blocks_ring_buffer,
      out rast)
      LANGUAGE PLPGSQL AS $$
        declare
        this_abi abi;
        BEGIN
          select * into this_abi from abi where
        select st_setsrid(
          st_setGeoReference(blocks_in.rast,blocks_in.west,blocks_in.north,abi_res,-abi_res,0,0),
                       goes_srid) INTO STRICT rast
        END
        $$;
    #+end_src
